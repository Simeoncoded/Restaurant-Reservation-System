...
namespace RestaurantReservationSystem.Repositories
{
    ...
    public class ReservationService : IReservationService
    {
        ...
        public async Task<List<DateTime>> GetAvailabilityAsync(DateTime requested, int partySize, CancellationToken ct = default)
        {
            if (partySize <= 0)
            {
                return new();
            ...
            //get candidates tables that can seat this party and are available
            var tables = await _context.Tables
                         .Where(t => t.Status == TableStatus.Available && t.Capacity >= partySize)
            ...
            //build all 30 min slots within open hours for that day
            var start = day + OpenTime;
            var end = start + CloseTime;

            var allSlots = new List<DateTime>();
            for (var t = start; t <= end; t = t.Add(Slot)) allSlots.Add(t);

            //Pull existing reservations for that day for the candidate tables 
            ...
            //A slot is available if at least ONE table is free at that time 
            ...
            foreach (var slot in allSlots)
            {
                ...
                //count how many tables are already taken at this slot 
                var takenAtSlot = existing.Count(e => e.Time == slotTime);

                //if not all tables are taken, we have at least one free table
